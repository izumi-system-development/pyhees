...
def get_Theta_hs_out_max_H_d_t(Theta_star_hs_in_d_t, Q_hs_max_H_d_t, V_dash_supply_d_t_i):
    ...
    c_p_air = get_c_p_air()
    rho_air = get_rho_air()
    # 元コードでは Theta_hs_out_max_H_d_t_limit は固定値(45.0)
    arr = Theta_star_hs_in_d_t + ((Q_hs_max_H_d_t * 10 ** 6) / (c_p_air * rho_air * np.sum(V_dash_supply_d_t_i[:5, :], axis=0)))
    return np.clip(arr, None, constants.Theta_hs_out_max_H_d_t_limit)
...
def get_Theta_hs_out_min_C_d_t(Theta_star_hs_in_d_t, Q_hs_max_CS_d_t, V_dash_supply_d_t_i):
    ...
    c_p_air = get_c_p_air()
    rho_air = get_rho_air()
    # 元コードでは Theta_hs_out_min_C_d_t_limit は固定値(15.0)
    arr = Theta_star_hs_in_d_t - ((Q_hs_max_CS_d_t * 10 ** 6) / (c_p_air * rho_air * np.sum(V_dash_supply_d_t_i[:5, :], axis=0)))
    return np.clip(arr, constants.Theta_hs_out_min_C_d_t_limit, None)
